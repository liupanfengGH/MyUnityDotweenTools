...
public static class UnityInteralStyle 
{
    [MenuItem("MyTools/Viewer/InternalStyle")]
    static public void ShowInternalStyleIcons()
    {
        var window = EditorWindow.GetWindow<UnityInteralStyleWindow>("Unity内置风格");
        window.position = new Rect(new Vector2(Screen.width / 2, Screen.height / 2), new Vector2(800, 600));
        window.Show();
    }

    [MenuItem("MyTools/Viewer/InternalIcons")]
    static public void ShowInternalTextrueIcons()
    {
        var window = EditorWindow.GetWindow<UnityInteralIconsWindow>("Unity内置纹理图片");
        window.position = new Rect(new Vector2(Screen.width / 2, Screen.height / 2), new Vector2(800, 600));
        window.Show();
    }
}

public class UnityInteralStyleWindow : EditorWindow
{
...
public class UnityInteralIconsWindow : EditorWindow
{
    ...
    private List<string> icons1 = new List<string>();

    private List<string> showIcons = null;

    void Awake()
    {
        icons.AddRange(iconsNames.Split(';'));
        icons1.AddRange(ico_list);
    ...
    void OnGUI()
    {
        ...
        foreach(var icon in showIcons)
        {
            ...
            if (GUILayout.Button(content, GUILayout.Width(80f),GUILayout.Height(80f)))
            {
                TextEditor textEditor = new TextEditor();
                textEditor.text = icon;
                textEditor.SelectAll();
                textEditor.Copy();
            }
            GUILayout.EndHorizontal();
